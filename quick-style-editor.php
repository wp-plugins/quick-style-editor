...
function qse_use_css () {
	...
	if ($qse['use']) {
		...
		$code .= str_replace('nav',$qse['navid'].' ',$item) . "{";
		for ($i=1; $i<=4; $i++) { if ($qse[$item]['value'.$i]) $code.= $qse[$item]['property'.$i].":".$qse[$item]['value'.$i]. " !important; "; }
		$code.= "}\r\n"; } 
...
function qse_options_page() {
	...
	<p><span style="color: red">Caveat:</span> This plugin only works if your theme uses the standard WordPress structure. If your theme has a different naming convention this the syles may not work.</p>
	...
	<p>Use the style editor: <input type="checkbox" name="use" value ="checked" ' . $qse['use'] . ' /></p>
	<h2>Body Styles</h2>
	<table>
	<tr>
	...
	foreach ($navigation as $item) {
	$content .= '<tr>
	<td><b>' . str_replace('nav','nav ',$item) . ' </b></td><td><b>{</b></td>';
	for ($i=1; $i<=4; $i++) {
	$content .= '<td><span><input name="' .$item.'property'.$i. '" type="text" size="8" value="' . $qse[$item]['property'.$i] . '"/></span><b>:</b></td>  
	<td><input name="' . $item.'value'.$i . '" type="text" size="14" value="' . $qse[$item]['value'.$i]  . '"/><b>;</b></td>'; }
	$content .='<td><b>}</b></td></tr>';
...
function qse_defaults () {
	$qse['use'] = 'checked';
	$qse['navid'] = 'nav';
	$qse['structure'] = 'header,content,primary,secondary,sidebar,footer';
	$qse['format'] = 'p,h1,h2,h3,h4';
	$qse['navigation'] = 'nav,navul,navli';
	$qse['custom'] = 'selector1,selector2';
	$qse['body'] = array(
